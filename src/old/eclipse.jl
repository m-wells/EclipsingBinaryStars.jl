_no_eclipse(Δ, R₁, R₂) = (R₁ + R₂) < Δ
_partial_eclipse(Δ, R₁, R₂) = abs(R₁ - R₂) < Δ < (R₁ + R₂)
_total_eclipse(Δ, R₁, R₂) = Δ < abs(R₁ - R₂)

function _eflag(ρ::Length, R₁::Length, R₂::Length, zsign::Int8)
    _no_eclipse(ρ, R₁, R₂) && return Int8(0)
...
struct Eclipse{T}
    ...
    function Eclipse(ρ::AU{T}, zsign::Int8, eflag::Int8, νc::Degree{T}) where T
        new{T}(ρ, zsign, eflag, νc)
    end

    function Eclipse(ρ::AU{T}, zsign::Int8, eflag::Int8, νc::Angle) where T
        new{T}(ρ, zsign, eflag, ustrip(T, u"°", νc)*u"°")
    end
...
function Base.show(io::IO, e::Eclipse)
    print(io, "(", compact(uconvert(u"Rsun", get_ρ(e))), ", ")
...
#TODO this is only approximate (1st order)
#get_pri_eclipse(b::Binary) = Eclipse(b, superior_conj(b))
#get_sec_eclipse(b::Binary) = Eclipse(b, inferior_conj(b))

function get_pri_eclipse(b::Binary)
    f(ν) = ustrip(proj_sep(b, ν))
    ν = minimize_newton(f, ustrip(u"rad", superior_conj(b)))
    ν = mod2pi(ν)
    Eclipse(b, ν*u"rad")
end

function get_sec_eclipse(b::Binary)
    f(ν) = ustrip(proj_sep(b, ν))
    ν = minimize_newton(f, ustrip(u"rad", inferior_conj(b)))
    ν = mod2pi(ν)
    Eclipse(b, ν*u"rad")
end
